...
// DescMatchMinAndMax 匹配带最大，最小长度限制的描述信息
func DescMatchMinAndMax(text string, min int, max int) (ok bool, err error) {
	if min > max {
		return false, errors.New("备注或描述" + utils.StatusText(utils.MinThanMaxErr))
	}
	if len(text) >= min && len(text) <= max {
		if matched, _ := regexp.MatchString(DescRegex, text); matched {
			return true, nil
		}
		return false, fmt.Errorf("备注或描述为%d-%d位中文字符字母或数字组合", min, max)
	}
	return false, fmt.Errorf("备注或描述为%d-%d位中文字符字母或数字组合", min, max)
}

// DescMatchMax 匹配带最大长度的描述信息
func DescMatchMax(text string, max int) (ok bool, err error) {
	if max < 0 {
		return false, fmt.Errorf("备注或描述" + utils.StatusText(utils.MaxLessZeroErr))
	}
	if len(text) <= max {
		if matched, _ := regexp.MatchString(DescRegex, text); matched {
			return true, nil
		}
		return false, fmt.Errorf("备注或描述为小于%d位的中文字符字母或数字组合", max)
	}
	return false, fmt.Errorf("备注或描述为小于%d位的中文字符字母或数字组合", max)
}

// MatchLetterNumMinAndMax 匹配带最大，最小长度的数字字母
func MatchLetterNumMinAndMax(text string, min int, max int, purpose string) (ok bool, err error) {
	if min > max {
		return false, errors.New(purpose + utils.StatusText(utils.MinThanMaxErr))
	}
	if len(text) >= min && len(text) <= max {
		if matched, _ := regexp.MatchString(LetNumRegex, text); matched {
			return true, nil
		}
		return false, fmt.Errorf(purpose+"为%d-%d位符字母或数字组合", min, max)
	}
	return false, fmt.Errorf(purpose+"为%d-%d位符字母或数字组合", min, max)
}

// MatchLetterNumMax 匹配带最大长度的数字字母
func MatchLetterNumMax(text string, max int, purpose string) (ok bool, err error) {
	if max < 0 {
		return false, errors.New(purpose + utils.StatusText(utils.MaxLessZeroErr))
	}
	if len(text) <= max {
		if matched, _ := regexp.MatchString(LetNumRegex, text); matched {
			return true, nil
		}
		return false, fmt.Errorf(purpose+"为小于%d位符字母或数字组合", max)
	}
	return false, fmt.Errorf(purpose+"为小于%d位符字母或数字组合", max)
}

// MatchChineseMinAndMax 匹配带最大最小长度的中文
...
// MatchChineseName 匹配中文名字
func MatchChineseName(text string, min int, max int) (ok bool, err error) {
	if min > max {
		return false, errors.New("中文名" + utils.StatusText(utils.MinThanMaxErr))
	}
	if len(text) >= min && len(text) <= max {
		if matched, _ := regexp.MatchString(ChineseNameRegex, text); matched {
			return true, nil
		}
		return false, errors.New(fmt.Sprintf(utils.StatusText(utils.ChineseNameErr), min, max))
	}
	return false, errors.New(fmt.Sprintf(utils.StatusText(utils.ChineseNameErr), min, max))
}

// MatchEnglishName 匹配英文名
func MatchEnglishName(text string, min int, max int) (ok bool, err error) {
	if min > max {
		return false, errors.New("英文名" + utils.StatusText(utils.MinThanMaxErr))
	}
	if len(text) >= min && len(text) <= max {
		if matched, _ := regexp.MatchString(EnglishNameRegex, text); matched {
			return true, nil
		}
		return false, errors.New(fmt.Sprintf(utils.StatusText(utils.EnglishNameErr), min, max))
	}
	return false, errors.New(fmt.Sprintf(utils.StatusText(utils.EnglishNameErr), min, max))
}

// MatchStrongPassword 强密码
func MatchStrongPassword(text string, min int, max int) (ok bool, err error) {
	if min > max {
		return false, errors.New("密码" + utils.StatusText(utils.MinThanMaxErr))
	}
	if len(text) >= min && len(text) <= max {
		if matched, _ := regexp.MatchString(StrongPassword, text); matched {
			return true, nil
		}
		return false, errors.New(fmt.Sprintf(utils.StatusText(utils.StrongPasswordErr), min, max))
	}
	return false, errors.New(fmt.Sprintf(utils.StatusText(utils.StrongPasswordErr), min, max))
}

// MatchMediumPassword 匹配中度密码
func MatchMediumPassword(text string, min int, max int) (ok bool, err error) {
	...
	if len(text) >= min && len(text) <= max {
		...
		return false, errors.New(fmt.Sprintf(utils.StatusText(utils.MediumPasswordErr), min, max))
	}
	return false, errors.New(fmt.Sprintf(utils.StatusText(utils.MediumPasswordErr), min, max))
}

// MatchNumStrMinAndMax 匹配纯数字字符串
...
// MatchNumStrFix 匹配定长的纯数字
func MatchNumStrFix(text string, fix int, purpose string) (ok bool, err error) {
	if fix < 0 {
		return false, errors.New(purpose + utils.StatusText(utils.FixLessZeroErr))
	}
	if len(text) == fix {
...
// MatchHexStrMinAndMax 匹配带最大最小长度的十六进制字符串
...
// MatchHexStrMax 匹配带最大长度的十六进制字符串
